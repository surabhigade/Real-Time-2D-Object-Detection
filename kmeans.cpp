...
/*
 ...
 labels: an allocated array of type int, the same size as the data, contains the labels when the function 
returns
...
*/
int kmeans( std::vector<cv::Vec3b> &data, std::vector<cv::Vec3b> &means, int *labels, int K, int 
maxIterations, int stopThresh ) {
    ...
    int istp = std::rand() % (data.size() % K);

    for(int i=0;i<K;i++) {
        int index = (istp + i*delta) % data.size();
    ...
    for(int k=0; k<tmeans.size(); k++) {
        if(tmeans[k][3] > 0) { // avoid division by zero
